...
export const updateEvent = (eventId, updatedData) => async (dispatch) => {
  try {
    genericApi.put(`/api/events/${eventId}`, updatedData).then((res) => {
      dispatch({
        type: "UPDATE_EVENT",
        payload: res.data,
      });
...
export const participantLeave = async (dispatch, eventId, userId) => {
  try {
    const res = genericApi
      .patch(`/api/events/${eventId}/removeParticipant`, { userId: userId })
      .then(() => {
        dispatch({
...
export const addComment = async (dispatch, eventId, user, text) => {
  try {
    const res = genericApi
      .patch(`/api/events/${eventId}/comments`, {
        comment: { user, text },
      })
      .then(() => {
        dispatch({
